(function() {
    'use strict';
    angular.module('pr.longpress', []).directive('onLongPress', [
        '$parse',
        '$timeout',
        function($parse, $timeout) {
            return {
                ...
                link: function($scope, $elm, $attrs) {
                    var timer;
                    var timerDuration =
                        (!isNaN($attrs.longPressDuration) && parseInt($attrs.longPressDuration)) ||
                        600;
                    // By default we prevent long press when user scrolls
                    var preventLongPressOnScroll = $attrs.preventOnscrolling
                        ? $attrs.preventOnscrolling === 'true'
                        : true;
                    ...
                    function onClick(evt) {
                        ...
                        if (
                            $scope.longPressSent &&
                            (!$attrs.preventClick || $attrs.preventClick === 'true')
                        ) {
                            evt.preventDefault();
                            evt.stopPropagation();
                    ...
                    function onMove(evt) {
                        ...
                        if (
                            (touchStartY !== undefined &&
                                ...
                                Math.abs(yPosition - touchStartY) > MAX_DELTA) ||
                            Math.abs(xPosition - touchStartX) > MAX_DELTA
                        ) {
                            $timeout.cancel(timer);
                        }
                    }
