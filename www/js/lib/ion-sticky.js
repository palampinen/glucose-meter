angular.module('ion-sticky', ['ionic']).directive('ionSticky', [
  '$ionicPosition',
  '$compile',
  '$timeout',
    ...
    return {
      ...
      link: function($scope, $element, $attr, $ionicScroll) {
        ...
        var createStickyClone = function($element) {
          ...
          $attr.ionStickyClass = $attr.ionStickyClass ? $attr.ionStickyClass : 'ion-sticky';
        ...
        var updateSticky = ionic.throttle(function() {
          var divHeight = $ionicPosition.offset(
            $element[0].getElementsByClassName($attr.ionSticky || 'item-divider')
          ).height;
          ...
          for (var i = 0; i < dividers.length; ++i) {
            ...
            if (
              $ionicPosition.offset(dividers[i]).top -
                $ionicPosition.offset($element).top -
                divHeight <
              0
            ) {
              ...
              if (
                ...
                $ionicPosition.offset(dividers[i + 1]).top -
                  ($ionicPosition.offset($element).top + dividers[i + 1].prop('offsetHeight')) +
                  divHeight >
                  0
              ) {
                //when clone covers new header
                active = dividers[i][0];
                break;
              }
            }
          ...
          if (lastActive != active) {
            removeStickyClone();
            lastActive = active;
            if (active != null) createStickyClone(angular.element(active));
          }
          //console.log(performance.now());
        }, 200);
